...
void ArucoTF::verifyCalibration(const int &marker_id) {
  ...
  // complete this section for Lab_8
  ...
  Eigen::MatrixXf covariance_matrix(7, 7);
  covariance_matrix.setZero();

  for (auto pose_error : pose_errors) {
    covariance_matrix += (pose_error - mean_error) *
                        (pose_error - mean_error).transpose();
  }

  covariance_matrix /= pose_errors.size();
